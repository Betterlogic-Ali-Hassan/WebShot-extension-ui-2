...
import { Pencil, Paintbrush, Highlighter, X, ChevronDown } from "lucide-react";
import Button from "@/components/ui/button";
import { cn } from "@/lib/utils";

interface PencilToolPopupProps {
  position: { top: number; left: number; width: number };
  isDarkMode: boolean;
  onClose: () => void;
  onToolSelect?: (tool: "pencil" | "brush" | "highlighter") => void;
  selectedTool?: "pencil" | "brush" | "highlighter";
...
export function PencilToolPopup({
  ...
  onToolSelect,
  toolbarPosition,
  selectedTool: propSelectedTool = "pencil",
}: PencilToolPopupProps) {
  // Tool selection state - use the prop if provided
  const [selectedTool, setSelectedTool] = useState(propSelectedTool);

  // Update the tool selection handler to call the callback
  const handleToolSelect = (tool: "pencil" | "brush" | "highlighter") => {
  ...
  // Add the same positioning improvements to PencilToolPopup
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {tools.map((tool) => (
            <button
              key={tool.id}
              onClick={() =>
                handleToolSelect(tool.id as "pencil" | "brush" | "highlighter")
              }
              className={cn(
                ...
                selectedTool === tool.id
                  ...
                  : isDarkMode
                  ? "text-gray-300 hover:bg-[#3A3A3A] hover:text-[#F5F5F5]"
        ...
        <div
          className={cn(
            "h-10 w-px bg-gray-300 dark:bg-gray-700 hidden",
            isToolbarLeft && "w-full h-[1px] block my-5"
          )}
        ></div>
        <div
          className={cn("flex items-center gap-4 ", isToolbarLeft && "mt-6")}
        >
          ...
          {/* Custom Stroke Size Dropdown */}
          <div className='relative' ref={strokeDropdownRef}>
            <button
              onClick={() => setIsStrokeDropdownOpen(!isStrokeDropdownOpen)}
              className={cn(
                "flex items-center justify-between gap-2 px-3 py-1.5 rounded-md border cursor-pointer transition-all duration-200 w-32",
                isDarkMode
          ...
          <Button
            className={cn(
              "rounded-full h-8 w-8 transition-colors duration-200 justify-center",
              isDarkMode
