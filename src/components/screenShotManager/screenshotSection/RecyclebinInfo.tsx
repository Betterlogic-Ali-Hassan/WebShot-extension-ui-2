...
export const RecycleBinInfo = () => {
  ...
  return (
    <div className='mb-5 p-3 rounded-lg flex items-center bg-background border border-border'>
      ...
      {screenshots.filter((s) => s.trash).length > 0 && (
        <button
          ...
          className='flex-shrink-0 ml-3 px-3 py-1.5 rounded-full text-sm transition-all duration-200 bg-error text-text-primary hover:bg-error/60'
        >
          Empty Bin
        </button>
      )}
